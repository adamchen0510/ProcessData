...
if __name__ == "__main__":
    ...
    df_tick = pd.read_csv(tick_csv, dtype=str, names=["time", "timestamp", "last", "volume", "ask_0_p", "ask_0_v", "bid_0_p", "bid_0_v"])
    df_zhubi = pd.read_csv(zhubi_csv, dtype=str, names=["exchange_time", "contract", "price", "bs", "amount", "exchange_timestamp", "time", "timestamp"])
    ...
    for tick_row in df_tick.itertuples():
        ...
        if tick_v > 0.0:
            ...
            for zhubi_row in df_zhubi[df_zhubi_start_index:]:
                print(f"zhubi_row: {zhubi_row}")
                zhubi_time = getattr(zhubi_row, "time")
                if zhubi_time == "time":
                    df_zhubi.drop(index=[zhubi_row[0], zhubi_row[0]], inplace=True)
                    continue
                if zhubi_row[0] < df_zhubi_start_index:
                    continue
                ...
                # print(f"zhubi_total_volume: {zhubi_total_volume}")
